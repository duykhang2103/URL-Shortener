...
export function createDocument(
  ...
  swaggerConfig: SwaggerConfig = SWAGGER_CONFIG,
): OpenAPIObject {
  ...
  http_adapter.use(
    `${configService.get<string>('baseUrl')}/api-docs`,
    (req: Request, res: Response, next: NextFunction) => {
      ...
      if (
        credentials?.name !== configService.get<string>('userSwagger') ||
        credentials?.pass !== configService.get<string>('passSwagger')
      ) {
        return unauthorizedResponse();
      }
      next();
