...
export abstract class BaseRepository<T> implements IBaseRepository<T> {
    ...
    async update(CHANNEL_ID: EntityId, data: any): Promise<T> {
        try {
            await this.repository.update(CHANNEL_ID, data);
            return this.findById(CHANNEL_ID);
        } catch (error) {
    ...
    async delete(CHANNEL_ID: EntityId): Promise<DeleteResult> {
        try {
            return await this.repository.delete(CHANNEL_ID);
        } catch (error) {
